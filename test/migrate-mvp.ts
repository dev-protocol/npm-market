...
contract('Migrate MVP', ([deployer]) => {
	describe('migration', () => {
		...
		it('migration all packages', async function(done) {
			...
			res.map(async ({metrics, property}) => {
				...
				expect(related2Metrics).to.be.equal(property)
				expect(author).to.be.equal(pkg!.user)
				expect(name).to.be.match(/^[a-z-]*$/)
				expect(symbol).to.be.match(/^[A-Z]*$/)
			})
			done()
		})
	})
